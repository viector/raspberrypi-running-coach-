...
//	std::cout <<" filteredIRValue, " << filteredIRValue << " irValue, " << irValue << std::endl;                             //////////////////////////////////////
//	return;

	int timeSinceLastIRHeartBeat = std::chrono::duration_cast<std::chrono::milliseconds>(timeCurrent - timeLastIRHeartBeat).count();
	//std::cout << "timeCurrent - timeLastIRHeartBeat" << timeCurrent << "  " << timeLastIRHeartBeat << endl;

	//std::cout << loopCount++ << "," << filteredIRValue << ": ";
	if (peakDetect(filteredIRValue)) {
		int _irBPM = 60000/timeSinceLastIRHeartBeat;
		//std::cout << timeSinceLastIRHeartBeat << " timeSinceLastIRHeartBeat " << "  BPM : " << _irBPM << std::endl;                                 /////////////////////////////////////
		latestIRBPM = _irBPM;
