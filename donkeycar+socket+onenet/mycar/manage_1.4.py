...
#from donkeycar.parts.onenet import OneNet
from donkeycar.parts.onenet_thread import OneNet_th
from demo import Wake
...
print(ip)
skt.close()
os.system("ssh -fCNR 9330:"+ip+":8887 sice@10.33.32.6")
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[] ):
    ...
    V.add(ctr, 
          ...
          outputs=['user/angle', 'user/throttle', 'user_mode', '_recording'],
          threaded=True)
    ...
    wake=Wake()
    V.add(wake,outputs=['audio/order','audio/mode','user/mode','recording'],threaded=True)

    serv=Server()
    #V.add(serv,outputs=['heart_rate'],threaded=True)
    V.add(serv,inputs=['audio/order','audio/mode'],outputs=['heart/rate','heart/throttle','heart/speed'],threaded=True)
    
    #oneN=OneNet()
    #V.add(oneN,inputs=['heart/rate','heart/speed'])

    oneN_th=OneNet_th()
    V.add(oneN_th,inputs=['heart/rate','heart/speed'],threaded=True)
    ...
    #    led.set_rgb(cfg.LED_R, cfg.LED_G, cfg.LED_B)        
        ...
        #V.add(LedConditionLogic(cfg), inputs=['user/mode', 'recording', "records/alert", 'behavior/state', 'modelfile/modified', "pilot/loc"],outputs=['led/blink_rate'])
    ...
    if model_path:
        ...
        V.add(DelayedTrigger(25), inputs=['modelfile/dirty'], outputs=['modelfile/reload'], run_condition="ai_running")
    ...
    class DriveMode:
        def run(self, mode, 
                    ...
                    heart_angle,heart_throttle,audio_mode):
            #if mode == 'user': 
            ...
            #else:
            if pilot_throttle==None:
                pilot_throttle=0.0
            if pilot_angle==None:
                pilot_angle=0.0

            if heart_throttle==None:
                heart_throttle=0.0

            #if abs(pilot_angle) > 0.1  and heart_throttle > 0.3 :
            #    heart_throttle=heart_throttle - (pilot_angle - 0.1)*0.5
            if audio_mode==1 :
                if heart_throttle >= 0.38 :
                   heart_throttle=0.38
            if audio_mode==2 :
                if heart_throttle >= 0.43 :
                   heart_throttle=0.43
            if audio_mode==3 :
                if heart_throttle >= 0.47 :
                   heart_throttle=0.47

            if heart_throttle > pilot_throttle and pilot_throttle >0.3:
                pilot_angle = pilot_angle * ( heart_throttle / pilot_throttle )
            return pilot_angle , heart_throttle 
            #return pilot_angle, pilot_throttle * cfg.AI_THROTTLE_MULT
        
    V.add(DriveMode(), 
          inputs=['user/mode', 'user/angle', 'user/throttle',
                  'pilot/angle', 'pilot/throttle','heart/angle','heart/throttle','audio/mode'], 
